...
# in Content.pm so can be called from either text or graphics context
=head3 textlabel

    $width = $txt->textlabel($x,$y, $font, $size, $text, %opts)

    $width = $gfx->textlabel($x,$y, $font, $size, $text, %opts)
...
Notice that this call may be made with either text context I<or> graphics
context. If already in text context, it will briefly leave it (go into graphics
context) in order to save and restore the graphics state, before returning to
(or going into) text context for outputting text.
...
(assuming left-justified positioning and no rotation).
