...
my %state = $pdf->init_state(       # various settings
...
for (my $pass_count=1; $pass_count <= $max_passes; $pass_count++) {
    ...
    $ppn = 0;     # physical page number 1,2,...
...
consequat. See <_ref tgtid="xpage" title="Explanation 1"> for more. Duis aute 
...
back at the very beginning of this document, <_ref tgtid="chap1" 
title="Chapter One (shortened)">. 
...
eu fringilla sit amet, finibus vel purus. <_ref tgtid="lesser" title="A Link to 
the Second Heading">. Nunc laoreet metus eget malesuada pharetra. Sed nulla 
enim, consequat non blandit id, pretium sed erat. Just for the halibut, let's
<_ref tgtid="back1" title="go back a little"> on this page. Donec in auctor 
...
#<_ref tgtid="xpage">. 
#And here is one using the target's natural text: <_ref tgtid="someother">.</p>
#
#<p>This is a forward reference with no title, target title, or natural text:
#<_ref tgtid="Linking">. It should say "[no title given]" as the link text.
#This is a forward reference to a non-existent target: <_ref tgtid="glotz">.
...
<_ref tgtid="lesser" title="Eat more of these delicious condiments.">
...
nulla rhoncus tristique. Remember the first heading <_ref tgtid="chap1" title="Big Shot">? There's a Named Destination right 
...
dapibus magna. Go <_ref tgtid="someother" title="back to this page heading">.
</p>
END_OF_CONTENT
...
<p>Link back to the heading, using its natural title: <_ref tgtid="chap1">.
Now link to one using its specified title: 
<_ref tgtid="Lorem" fit="xyz,%x,%y,2">.</p>
...
<_ref tgtid="#foo" 
title="same PDF"> with a Named Destination target. Then link to it
<_ref tgtid="##1" fit="xyz,50,600,undef"
title="first page"> at x,y = 50,600 with xyz fit. Finally, 
link to <_ref tgtid="##1" title="that page" fit="fitv,200"> on page 1 with a 
specified fit.</p>
...
<_ref tgtid="resources/040_annotation.pdf#bar" 
title="finding Nemo"> with a Named Destination target. Then link to it by
<_ref tgtid="resources/040_annotation.pdf##2"
fit="xyz,50,600,2.5"
title="on second page"> at a specified x,y with xyz fit at a certain location.
Finally, link to
<_ref tgtid="resources/040_annotation.pdf##2"
title="that page" fit="fith,200"> with a specified fit.</p>
END_OF_CONTENT
